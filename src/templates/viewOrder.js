...
import StoreDashComponent from '../components/storeDashComponent'
import ModelBoxTemplate from './modelBox';
//import "react-datepicker/dist/react-datepicker.css";
    ...
    const orderList = Object.keys(this.props.orders).map( (key, index) =>
        ...
        <tr key={key}>
          ...
          <td className="cust-name">{orderArrayObj[key].customerName}</td>
           <td className="cust-name">{orderArrayObj[key].ordermode}</td>
          ...
          <td className="retrn-tab-btns">
          
          
          {(() => {
          switch (orderArrayObj[key].status){
            //case "confirmed":   return <><button className="btn-danger btn-sm initiate-rtrn-btn" onClick={()=>this.cancelOrder(orderArrayObj[key].order_id,orderArrayObj[key].mobile,orderArrayObj[key].orderNo,orderArrayObj[key].customerName,orderArrayObj[key].orderDate,orderArrayObj[key].paymentChannel,key)}>{orderArrayObj[key].action}</button><button className="btn-primary btn-sm cancel-rtrn-btn" onClick={()=>this.onModalShow(orderArrayObj[key].order_id,orderArrayObj[key].orderNo,key)}>Cancel</button></> ;
            case "confirmed":   return <><button className="btn-danger btn-sm initiate-rtrn-btn" onClick={()=>this.onAcceptModalShow(orderArrayObj[key].order_id,orderArrayObj[key].orderNo,key,orderArrayObj[key])}>{orderArrayObj[key].action}</button><button className="btn-primary btn-sm cancel-rtrn-btn" onClick={()=>this.onModalShow(orderArrayObj[key].order_id,orderArrayObj[key].orderNo,key)}>Cancel</button></> ;
            case "refundinitited": return  <button disabled className="btn btn-sm btn-warning">Refund Intiated</button>;
            ...
            //default:      return <button disabled className="btn-danger btn-sm" onClick={()=>this.cancelOrder(orderArrayObj[key].orderNo)}>{orderArrayObj[key].action}</button> ;
            default:      return <button disabled className="btn btn-danger btn-sm" >{orderArrayObj[key].action}</button> ;
          }
